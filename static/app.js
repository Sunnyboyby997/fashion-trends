...
async function waitForUpdate(timeoutMs = 240000) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    let s = null;
    try { s = await fetchJSON("data/status.json"); } catch { /* 忽略轮询错误 */ }
...
async function loadStatus() {
  try {
    ...
    $("#lastUpdate").textContent = s.last_update
      ? `最近更新 ${s.last_update} · ${s.schedule.mode === "daily" ? "每日" : s.schedule.mode === "weekly" ? "每周" : "手动"} ${s.schedule.time} 静态快照`
      : "最近更新 ——";
